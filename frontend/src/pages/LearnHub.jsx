...
export default function LearnHub() {
  ...
  const completedCount = sortedLessons.filter((l) => progress[l.slug]).length;
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-sm text-slate-400">
            {completedCount} of {sortedLessons.length} completed
          </span>
        </div>
        <div className="w-full bg-slate-700 rounded-full h-2.5">
          <div
            ...
            style={{
              width: `${sortedLessons.length > 0 ? (completedCount / sortedLessons.length) * 100 : 0}%`,
            }}
          />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {sortedLessons.map((lesson) => {
            ...
            const isStartHere = lesson.startHere && !done;
            return (
              <Link
                key={lesson.slug}
                ...
                className={`group relative block rounded-lg border p-5 transition-all hover:scale-[1.01] ${
                    ...
                    : isStartHere
                      ? 'border-amber-500/60 bg-amber-900/10 hover:border-amber-400/70 ring-1 ring-amber-500/30'
              ...
              >
                ...
                {isStartHere && (
                  <span className="absolute top-3 right-3 rounded-full bg-amber-600 px-2.5 py-0.5 text-xs font-semibold text-white">
                    Start here
                  </span>
                )}
